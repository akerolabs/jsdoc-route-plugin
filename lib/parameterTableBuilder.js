...
function buildTableEntry(param, columnInfo) {
  ...
  return `<tr><td class="name"><code>${param.name}</code></td><td class="type">${type}</td>${attributeCell}${defaultCell}<td class="description last">${param.description}</td></tr>`;
}
...
function buildTableHeader(columnInfo) {
  ...
  return `<thead><tr><th>Name</th><th>Type</th>${attributeHeader}${defaultHeader}<th class="last">Description</th></tr></thead>`;
}
...
exports.build = function(title, params){
  ...
  return `<h5>${title}:</h5><table class="params">${buildTableHeader(columnInfo)}${paramTableEntries.join('')}</table>`;
}